...
Algae.Shannon.Diversity.BW=ggplot(Algae.Shannon.Diversity.Summary, aes(x=Site,y=mean,fill=Substrate))+
  geom_bar(stat="identity", color="black", position=position_dodge())+
  ylab("Shannon Diversity (H')")+
  xlab("Site")+
  theme_classic()+
...
OTU=apply(Algae.df.6[,6:62]>=observationThreshold, 2, sum,na.rm=T) #check by row to see if each taxa present or not
OTU.0=apply(Algae.df.6[,6:62],2,sum,na.rm=T)
...
OTU.df <- OTU.df[-c(19,29,39),] #Remove rows that are controls --> Anna2020SCB23S, Anna2031SCD23S, Anna2041SCE23S
OTU.df=separate(OTU.df,name,into=c("Run_Info","Site","Substrate","Rep","Gene"), sep=c(-7,-5,-4,-3))
OTU.df$Run_Info=NULL
OTU.df$Gene=NULL
OTU.df$Site=factor(OTU.df$Site,levels=c("AZ","MD","WA","HC","NB","SP"),labels=c("Arizona","Maryland","Washington","Higgins Creek","N Br Chicago R","Springbrook Creek"))
OTU.df$Substrate=factor(OTU.df$Substrate,levels=c("P","F","T"),labels=c("PVC","Foam","Tile"))

#2-Way ANOVA on OTU Richness

OTU.aov<-aov(value~Site*Substrate, data=OTU.df)
summary(OTU.aov)
